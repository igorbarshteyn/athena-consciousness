...
// ATHENA (r20): inside talk-llama, whisper's libcommon already embeds a miniaudio
// implementation (common-whisper.cpp), and a second one here collides at link time
// (ma_atomic_global_lock). Athena feeds mtmd images only — her audio path is whisper —
// so ATHENA_MTMD_NO_AUDIO_FILES compiles the audio-FILE decode helpers out entirely.
// Upstream behavior is untouched when the macro is undefined.
#ifndef ATHENA_MTMD_NO_AUDIO_FILES
...
#endif // ATHENA_MTMD_NO_AUDIO_FILES
...
#ifdef ATHENA_MTMD_NO_AUDIO_FILES
// ATHENA (r20): audio-file decoding compiled out (see note at the miniaudio include).
// Buffers are never treated as audio; every input proceeds down the image path.
static bool is_audio_file(const char * /*buf*/, size_t /*len*/) { return false; }
static bool decode_audio_from_buf(const unsigned char * /*buf_in*/, size_t /*len*/,
                                  int /*target_sampler_rate*/,
                                  std::vector<float> & /*pcmf32_mono*/) { return false; }
#else
